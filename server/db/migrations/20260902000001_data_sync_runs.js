 ...
 * this table; freshness (MARKET_STALE_DAYS) reads the latest ok = true row here.
 *
 ...
 * every future job. The health read pulls only detail->>'matched'.
 *
...
exports.up = async function (knex) {
  await knex.schema.createTable(TABLE, (t) => {
    ...
    // instant it captured before the upstream fetch); finished_at defaults to the
    // write instant so a single INSERT at the end records the whole run.
  ...
  // The status read wants the latest row for a job:
  // WHERE job = $1 ORDER BY finished_at DESC LIMIT 1. Freshness wants the latest
  // ok = true row for the ADP job. One composite index serves both.
  await knex.raw(
