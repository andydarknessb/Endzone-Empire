...
const ENABLED = process.env.DRAFT_ROUNDS_PG_TESTS === '1';
const URL_VARS = ['DATABASE_URL', 'DATABASE_URL_RUNTIME', 'DATABASE_URL_MIGRATIONS'];
const urlLeak = URL_VARS.filter((k) => process.env[k]);

if (!ENABLED) {
  test('draft_rounds backfill PG tests (skipped: DRAFT_ROUNDS_PG_TESTS not set — CI migration-smoke runs these)', { skip: true }, () => {});
