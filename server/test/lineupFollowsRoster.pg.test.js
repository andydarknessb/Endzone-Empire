...
if (!ENABLED) {
  ...
  async function survivingRows(teamId) {
  ...
  test.before(async () => {
    ...
    // Two managers, because a matchup needs two teams and a team is one per
    // manager per league.
    ...
    const teams = await pool.query(
      `INSERT INTO "teams" ("league_id", "owner_id", "name")
       VALUES ($1, $2, 'PG Departing'), ($1, $3, 'PG Opponent')
       RETURNING "id", "name"`,
      [league.rows[0].id, users.rows[0].id, users.rows[1].id]
    );
    teamA = teams.rows.find((t) => t.name === 'PG Departing').id;
    teamB = teams.rows.find((t) => t.name === 'PG Opponent').id;
    ...
    // Team B: a settled week. Its own matchup for the current week is final,
    // so even its no-game player's row is the record of a week as played.
    await pool.query(
      `INSERT INTO "matchups" ("league_id", "season", "week", "home_team_id", "away_team_id", "final")
       VALUES ($1, $2, $3, $4, $5, true)`,
      [leagueIds[0], SEASON, CURRENT_WEEK, teamB, teamA]
    );
    await pool.query(
      `INSERT INTO "lineup_entries" ("league_id", "team_id", "player_id", "season", "week", "slot")
       VALUES ($1, $2, $3, $4, $5, 'BENCH')`,
  ...
  test('the cleanup migration is a no-op on a database with nothing stale', async () => {
    // Re-running it is the "empty database" case in the only form that can
    // be observed here: there is nothing left to match, and it must not
    // throw or take anything else with it.
