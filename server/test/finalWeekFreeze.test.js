...
function createWorld({
  ...
  const handlers = [
    ...
    [/^SELECT 1 FROM "matchups"/, (text, [leagueId, season, week, teamId]) => ({
      rows: state.matchups
...
test('#106 best ball: a player acquired after a final week never joins its re-score', async (t) => {
  ...
  // The mechanism: no week-8 row was ever created for him.
  assert.deepEqual(
...
test('#106 standard: a copied-forward starting slot cannot re-score a final week', async (t) => {
  // P started at RB for team B in week 7, was dropped, and is re-acquired in
  // week 8 after the games. materializeLineup's copy-forward would hand him
  // week 7's RB slot, which a standard league counts.
...
test('#106 control: the same acquisition DOES score in a live best-ball week', async (t) => {
...
test('#106 control: the same acquisition DOES score in a live standard week', async (t) => {
  const world = createWorld({
    bestBall: false,
    week8Final: false,
    priorWeekEntries: [
...
test('#106 an acquisition made AFTER finality still benches him in the new week', async (t) => {
  // The routine Tuesday window: week 8 is final, the commissioner has not
  // advanced, and team B wins P on waivers. Week 8 stays frozen; week 9 picks
  // him up on its first touch with no earlier row to copy, so BENCH.
  const world = createWorld({ bestBall: false });
  world.fake.install(t);
