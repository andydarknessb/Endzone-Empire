...
function autodraftPool({ isCommissioner }) {
...
test('POST autodraft refuses a Team owner who is not a commissioner', async (t) => {
  const fake = autodraftPool({ isCommissioner: false }).install(t);

  const response = await toggle(MANAGER_ID, true);

  assert.equal(response.status, 403, JSON.stringify(response.body));
  assert.equal(response.body.error, 'only the commissioner can change autodraft');
...
test('POST autodraft lets a commissioner change any Team', async (t) => {
  const fake = autodraftPool({ isCommissioner: true }).install(t);
