...
// Kept whole (not destructured), the same test-seam convention playerCard.
// service.js documents: a destructured binding is captured at require time
// and a test can no longer mock it afterwards, and leagueDetail.test.js
...
// GET /api/league/:id — league detail: teams, owners, roster counts. The
// query, authorization and shaping logic lives in leagueDetail.service.js
// (#1499); this handler is a call plus the coded-refusal -> status mapping.
router.get('/:id', async (req, res) => {
  const leagueId = intParam(req.params.id);
  if (!leagueId) return res.status(400).json({ error: 'league id must be a positive integer' });
  try {
    const detail = await leagueDetailService.leagueDetail({ leagueId, viewer: req.user.id }, { db: pool });
    res.json(detail);
  } catch (error) {
    if (error instanceof leagueDetailService.LeagueDetailError) {
