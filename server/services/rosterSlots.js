 ...
 * the bundle to src/), so a client mirror exists and this is what it is
 * measured against:
 *
 ...
 *
 * src/lib/lineupAttention.js used to keep a second client mirror,
 * `DEFAULT_STARTER_SLOT_ORDER` (the starter KEYS only), pinned here by
 * lineupAttention.parity.test.js. #1210 deleted that mirror (ADR 0029's
 * sibling concern: a default guesses at a fantasy-standard shape that could
 * mis-place IDP starters); lineupAttention now inspects only a league's own
 * configured slots, with no fallback to this module's shape at all.
 *
 * So do not treat this as the only copy: changing the contents here does not
 * propagate to the remaining client mirror. The whole-object
 * templates.parity.test.js is what fails on a count or eligiblePositions
 * drift in templates.js.
 *
 * There is no second, hand-kept client copy. The commissioner roster form
 ...
 * leaf. So there is one client copy - templates.js (whole object) - and it is
