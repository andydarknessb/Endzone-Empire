...
export default function KeeperPanel({ league, teams, keepers, keeperCandidates, frozen, onSaveLeague, onSaveKeepers, saving, onSettingsDirtyChange, onAssignmentsDirtyChange }) {
  ...
  const keeperLockError = lockMode === 'custom' && lockAt && new Date(lockAt).getTime() <= Date.now()
  ...
  return <Stack spacing={2}>
    ...
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="flex-start"><TextField label="Keepers per team" type="number" disabled={frozen || !enabled} value={count} inputProps={{ min: 0, max: draftRounds }} error={Boolean(keeperCountError)} helperText={keeperCountError} onChange={(event) => setCount(event.target.value)} /><Box><Typography variant="subtitle2" component="p" id="keeper-lock-label">Keeper lock</Typography><RadioGroup row aria-labelledby="keeper-lock-label" value={lockMode} onChange={(event) => setLockMode(event.target.value)}><FormControlLabel value="draft" disabled={frozen || !enabled} control={<Radio />} label="Draft date" /><FormControlLabel value="custom" disabled={frozen || !enabled} control={<Radio />} label="Custom" /></RadioGroup>{lockMode === 'custom' && <TextField label="Keeper lock date and time" type="datetime-local" size="small" InputLabelProps={{ shrink: true }} sx={(theme) => ({ mt: 1, colorScheme: theme.palette.mode })} value={lockAt} disabled={frozen || !enabled} error={Boolean(keeperLockError)} helperText={keeperLockError} onChange={(event) => setLockAt(event.target.value)} />}</Box></Stack>
    <Box><Button variant="contained" disabled={frozen || saving || Boolean(keeperCountError) || Boolean(keeperLockError)} onClick={requestSettingsSave}>Save keeper settings</Button></Box>
