...
test('maps reason "on bye" to a bye bar', () => {
  expect(weeksForSparkline([{ week: 9, reason: 'on bye' }])).toEqual([{ week: 9, kind: 'bye' }]);
});

test('maps any other reason to an unavailable bar carrying the reason', () => {
  expect(weeksForSparkline([{ week: 3, reason: 'on IR' }])).toEqual([
    { week: 3, kind: 'unavailable', reason: 'on IR' },
  ]);
