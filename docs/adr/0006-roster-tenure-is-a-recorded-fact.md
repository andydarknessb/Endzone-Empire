...
DELETE, so no caller, including the bulk league-reset deletes that bypass the
service chokepoints, can forget to write it. The settle pass and the
...
- **Call-site writes at the six acquisition seams and the removal chokepoint.**
  Rejected: it is correct only while every caller remembers, and the two bulk
  resets already bypass the chokepoint.
